...
def enroll_multiple_frames_fast(frames: List[np.ndarray], nik: int, min_embeddings: int = 5) -> Tuple[int, str]:
    """Enroll multiple frames - FAST VERSION (no photo save)"""
    enrolled = 0
    
    app = _get_face_app()
    if app is None:
        return 0, "Face detection unavailable"
    ...
    for idx, frame in enumerate(frames):
        try:
            ...
            if face_roi.size > 0:
                ...
                if is_blurry(gray, 50.0):
                    continue
            
            # Get embedding langsung
            embedding = _normalize_embedding(largest.embedding)
            quality = 0.5 + (largest.det_score * 0.5)
            
            # ✅ FAST: Save embedding ONLY, no photo
            if save_embedding(nik, embedding, quality, photo_bgr=None):  # photo_bgr=None!
                global _embeddings_db
                if nik not in _embeddings_db:
                    _embeddings_db[nik] = []
                _embeddings_db[nik].append(embedding)
                enrolled += 1
                logger.info(f"Frame {idx+1}: Enrolled NIK={nik} (fast mode)")
                
                if enrolled >= min_embeddings:
                    break
    ...
    if enrolled == 0:
        return 0, "No valid faces detected"
